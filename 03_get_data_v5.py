...
while True:
    ...
    if solved_indices.index(given[0]) not in solved_triangle:
        print(f"You have overwritten the value for {solved_indices[index]}")

    solved_triangle[index] = given[1]

    # check we have at least one side
    has_side = any(isinstance(item, (int, float)) for item in solved_triangle[:3])
    ...
    number_count = sum(isinstance(item, (int, float)) for item in solved_triangle)

    if has_side and number_count >= 2:
        break

print(solved_triangle)

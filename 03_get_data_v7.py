...
def get_given():
    ...
    while True:
        ...
        side_count = sum(isinstance(item, (int, float)) for item in given_values[:3])

        if side_count == 2:

            # check that sides a / b are smaller than the hypotenuse
            # if we have two sides
            if given_values[0] != "a" and given_values[2] != "c":
                if given_values[2] <= given_values[0]:
                    status = "not ok"
            elif given_values[1] != "b" and given_values[2] != "c":
                if given_values[2] <= given_values[1]:
                    status = "not ok"

            # tell user to over-write a side if the hyptenuse is too small.
            if status == "not ok":
                print("Your hypotenuse is too small.  Please over-write a side value")
                status = "ok"
                continue
